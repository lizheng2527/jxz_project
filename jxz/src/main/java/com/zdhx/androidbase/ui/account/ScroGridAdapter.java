...
public class ScroGridAdapter extends BaseAdapter {
    ...
    public ScroGridAdapter(List<String> list, Context context, boolean flag){
        this.list = list;
    ...
    public View getView(int i, View view, ViewGroup viewGroup) {
        ...
        if (view == null){
            vh = new ViewHolder();
            view = inflater.inflate(R.layout.fragment_scro_grid_item,null);
        ...
        if (!ECApplication.getInstance().getCurrentUser().getType().equals("2")){
            vh.tvLine.setVisibility(View.GONE);
        }
        return view;
    }
    class ViewHolder{
